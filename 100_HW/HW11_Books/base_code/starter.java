...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				...
				String textFile2 = sc.nextLine();
				textFile = textFile2;
				String[] arr2 = fillArray(textFile);
				arr = arr2;
			}else if(choice.equals("check")){
				System.out.println("What word/phrase would you like to check?");
				String word = sc.nextLine();
				System.out.println("There are " + checksArray(arr, word) + " many '" + word + "' in " + textFile);
	...
	*/
	public static String mostCommon(String[] arr){
		int [] uniqueWords = new int[arr.length];
		for(int i = 0; i < arr.length; i++){
		...
		int highestNum = uniqueWords[0];
		int mostCommonWord = 0;
		for(int j = 1; j < arr.length; j++){
			if(uniqueWords[j] > highestNum){
				highestNum = uniqueWords[j];
				mostCommonWord = j;
			}
		}
		
		return arr[mostCommonWord];
	}
